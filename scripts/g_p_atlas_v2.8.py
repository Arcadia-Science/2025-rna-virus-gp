#
# Description:
# This is an implementation of the G-P Atlas method, v2.8.
# This version tests the "Fitness as a Phenotype" hypothesis using a
# log-transformed fitness value to address the scale mismatch problem.
#
# v2.8 Changelog:
# - Trains on log-transformed fitness data.
# - Reverses the log-transformation during evaluation to calculate R^2
...
def main():
    ...
    train_file = os.path.join(vabs.dataset_path, "train.pk")
    test_file = os.path.join(vabs.dataset_path, "test.pk")
    ...
    with torch.no_grad():
        for norm_phens, gens in tqdm(test_loader_geno, desc="Evaluating Model"):
            ...
            # --- NEW: Reverse the log transform for evaluation ---
            # Un-normalize first
            pred_phens = pred_phens_norm * pheno_std + pheno_mean
            true_phens = norm_phens * pheno_std + pheno_mean
            
            # Reverse log-transform on the fitness column (index 16)
            pred_phens[:, 16] = torch.expm1(pred_phens[:, 16])
            true_phens[:, 16] = torch.expm1(true_phens[:, 16])
            # --- End of change ---
