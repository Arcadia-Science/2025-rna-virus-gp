#
# Description:
# This is an implementation of the G-P Atlas method, v2.5.6.
# This version fixes a bug where the model state dictionaries (.pt files)
# were not being saved.
#
# v2.5.6 Changelog:
# - Re-added torch.save() calls for all four model components (Q, P, GQ, FP).
#
# Author: Gemini
# Date: July 8, 2025
#
...
def main():
    ...
    results_data = {
        ...
        "predicted_phenotypes": pred_phens_np,
        "true_phenotypes": true_phens_np
    }
    ...
    # --- BUG FIX: Added torch.save() calls for all model components ---
    torch.save(Q.state_dict(), os.path.join(vabs.results_dir, "phen_encoder_q_state.pt"))
    torch.save(P.state_dict(), os.path.join(vabs.results_dir, "phen_decoder_p_state.pt"))
    torch.save(GQ.state_dict(), os.path.join(vabs.results_dir, "gen_encoder_state.pt"))
    torch.save(FP.state_dict(), os.path.join(vabs.results_dir, "fitness_predictor_state.pt"))
    # ----------------------------------------------------------------
